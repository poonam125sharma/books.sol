...
contract Buy_Novels {
    struct Novel {
        ...
        string title;
        string author;
        uint256 txnFee;         // Transaction fee in wei
        uint256 price;      // Novel price in wei
        bool isAvailable;   // If novel is available for buying
        address addingAddress;      // Address of the person who added this novel
        address buyerAddress;       // Address of the person who bought this novel
    }
    ...
    constructor(uint256 _id, string memory _title, string memory _author, uint256 _txnFee, uint256 _price) {
        address sender = msg.sender;
        novels[_id].id = _id;
        novels[_id].title = _title;
        novels[_id].author = _author;
        novels[_id].txnFee = _txnFee;
    ...
    function addNovel(uint256 _id, string memory _title, string memory _author, uint256 _txnFee, uint256 _price) public {
    ...
    function buyNovel(uint256 _id) public payable {
        ...
        // Assign the boughtAddress to the address of the person who bought this novel
        novels[_id].buyerAddress = msg.sender;
    }
    ...
    function returnNovel(uint256 _id) public payable {
        ...
        require(novels[_id].buyerAddress == msg.sender, "This novel was not bought by this address");
        
        uint256 returnPrice = novels[_id].price - novels[_id].txnFee;
        
        payable (novels[_id].buyerAddress).transfer(returnPrice);
